...
<c:if test="${sessionScope.id eq 'admin'}">
	<a href='<c:url value="/board/notice/write" />'><input type="button" value="글쓰기" class="btn"></a>
</c:if>
</form>
</div>
<div class="clear"></div>
...
<c:if test="${pb.startPage > pb.pageBlock }">
	<a href='<c:url value="/board/notice/main?pageNum=${pb.startPage-pb.pageBlock}"/>'>Prev</a>
</c:if>
<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage}" step="1">
	<a href='<c:url value="/board/notice/main?pageNum=${i}"/>'>${i}</a>
</c:forEach>
<c:if test="${pb.endPage > pb.pageBlock }">
	<a href='<c:url value="/board/notice/main?pageNum=${pb.startPage+pb.pageBlock}"/>'>Next</a>
</c:if>
</div>
</article>
<!-- 게시판 -->
